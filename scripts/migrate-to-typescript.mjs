...
function addTypeImports(content, filePath) {
  ...
  if (content.includes('authenticateUser') || content.includes('AuthenticatedEvent') || content.includes('defineEventHandler')) {
...
async function main() {
  ...
  const results = [];
  for (const jsFile of filesToMigrate.slice(0, 10)) { // Limit to first 10 files
