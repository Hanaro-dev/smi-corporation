<template>
  <div v-if="page" class="page-content">
    ...
    <!-- Le contenu est sanitisé avec DOMPurify avant le rendu -->
    ...
    <div v-if="childPages.length > 0" class="mt-8 border-t pt-4">
      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <NuxtLink :to="`/${childPage.slug}`" class="block">
    ...
    <div v-if="breadcrumbs.length > 1" class="mb-6 flex items-center text-sm">
      <ul class="flex flex-wrap items-center">
        ...
        >
          ...
          <span v-if="index < breadcrumbs.length - 1" class="mx-2 text-gray-500"
            >/</span
          >
        </li>
      </ul>
...
const fetchRelatedPages = async () => {
  ...
  await Promise.all([fetchChildPages(), fetchParentPage(), buildBreadcrumbs()]);
};
