...
const processCustomComponents = () => {
  ...
  orgCharts.forEach(async (element) => {
    ...
    if (!id) {
      element.innerHTML =
        '<p class="text-yellow-600">ID d\'organigramme manquant</p>';
      return;
    }
  ...
  galleries.forEach(async (element) => {
    ...
    if (!ids?.length) {
      element.innerHTML =
        '<p class="text-yellow-600">Aucun ID d\'image spécifié</p>';
      return;
    }
    ...
    try {
      // Indicateur de chargement
      element.innerHTML =
        '<div class="grid grid-cols-2 md:grid-cols-3 gap-4"><div class="animate-pulse bg-gray-200 rounded h-32"></div></div>';
  ...
  breadcrumbs.forEach((element) => {
    try {
      ...
      element.innerHTML =
        '<span class="text-gray-500">Navigation indisponible</span>';
    }
  });
};
...
const renderCustomBreadcrumb = () => {
  ...
  const breadcrumbItems = pathSegments.map((segment, index) => {
    ...
    const label =
      segment.replace(/-/g, " ").replace(/_/g, " ").charAt(0).toUpperCase() +
      segment.slice(1);
