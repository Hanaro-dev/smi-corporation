<template>
  <div>
    ...
    <form class="flex items-center gap-2 mb-4" @submit.prevent="uploadImage">
      <input type="file" accept="image/*" class="block" @change="onFileChange">
      <button type="submit" class="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Envoyer</button>
    </form>
    <div v-if="imageUrl" class="flex flex-col items-start gap-2">
      <img :src="imageUrl" alt="Image uploadée" class="max-w-xs rounded shadow" >
      <button class="px-4 py-1 bg-red-600 text-white rounded hover:bg-red-700" @click="deleteImage">Supprimer</button>
    </div>
  </div>
</template>
